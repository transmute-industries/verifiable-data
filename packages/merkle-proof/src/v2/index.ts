...
const addEdge = ( tree: Array<Buffer[]>, level: number, source: number, links: any) =>{
...
export const encodeProofAsObject = (root:Buffer, proof: Sibling[], member:Buffer) =>{
  const nodes:AutographNode[] = [];
  const links:AutographEdge[] = [];
  ...
  nodes.push({id: base64url.encode(sha256(member)), label: base64url.encode(sha256(member))})
  proof.forEach((s)=>{
...
export const addProofToTree = (treeGraph: Autograph, proofGraph: Autograph) =>{
  proofGraph.nodes.forEach((node)=>{
    const isNodeInTree = treeGraph.nodes.find((n)=>n.id === node.id) !== undefined
    if (!isNodeInTree){
...
export const encodeProofAsUrn = (root:Buffer, proof: Sibling[], member: Buffer): string =>{
...
export const proofUrnToJson = (proofUrn: string): string =>{
  ...
  return JSON.stringify({
    type, encoding, hash, root, directions, nodes
  }, null, 2)
}
...
const getProofFromUrn = (urn: string)=>{
  ...
  return {root, proof: auditPath.reverse(), member: base64url.toBuffer(member) }
}
